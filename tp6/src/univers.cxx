...
*/
Univers::Univers(int dim, int nbParticules, Vecteur ld, double rcut, LIMITE condition_limite, bool gravite):
    dim(dim), nbParticules(nbParticules), ld(ld), rcut(rcut), condition_limite(condition_limite), gravite(gravite)
    {
...
std::vector<Vecteur> Univers::calcul_forces(double epsilon, double sigma) {
    ...
    for (Particule &p1 : particules) {
        ...
        if(condition_limite == LIMITE::REFLEXION_POTENTIEL){
            ...
            double dist_y_haut = ld.getY() - p1.getPosition().getY();
            double dist_y_bas = p1.getPosition().getY();
            double dist_z_haut = ld.getZ() - p1.getPosition().getZ();
            double dist_z_bas = p1.getPosition().getZ();

            double force_x = 0;
            double force_y = 0;
            double force_z = 0;
            Vecteur pos= p1.getPosition();

            auto force_lj_mur = [&](double dist, int sign) {
                if (dist < rcut && dist > 1e-12) {
                    double inv_2r = sigma / (2.0 * dist);
                    double s6 = pow(inv_2r, 6);
                    return sign * 24.0 * epsilon / (2.0 * dist) * s6 * (1 - 2 * s6);
                }
                return 0.0;
            };

            if (dist_x_gauche < rcut) {
                force_x += force_lj_mur(ld.getX() - pos.getX(), -1);
            }
            if (dist_x_droite < rcut) {
                force_x += force_lj_mur(pos.getX(), +1);
            }
            if (dist_y_haut < rcut) {
                force_y += force_lj_mur(ld.getY() - pos.getY(), +1);
            }
            if (dist_y_bas < rcut) {
                force_y += force_lj_mur(pos.getY(), -1);
            }
            if (dist_z_haut < rcut) {
                force_z += force_lj_mur(ld.getZ() - pos.getZ(), -1);
            }
            if (dist_z_bas < rcut) {
                force_z += force_lj_mur(pos.getZ(), +1);
            }
            force_mur = Vecteur(force_x, force_y, force_z);
        ...
        forces[id1] += force_mur;
        forces[id1] += force_gravite;
    }
...
bool Univers::update_part(Particule& p, Vecteur& v) {
    ...
    if(condition_limite == LIMITE::ABSORPTION){
        ...
        auto periodicite = [](double coord, double limite) {
            double result = std::fmod(coord, limite);
            if (result < 0) result += limite; // pour avoir une valeur toujours positive
            return result;
        };
...
void Univers::testReflex(Vecteur vit, double mas) {
